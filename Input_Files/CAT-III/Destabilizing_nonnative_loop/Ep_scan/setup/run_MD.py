...
sim_steps = int(np.ceil(1*microseconds / timestep / 5000) * 5000)
fbsolu = 0.05/picosecond
temp = temp*kelvin

psf_pmd = pmd.load_file(psffile)
psf = CharmmPsfFile(psffile)
cor = pmd.load_file(corfile)
...
vec = list(simulation.context.getState(getVelocities=True).getVelocities())
fo = open(vecfile, 'r')
atom_idx = 0
for line in fo:
